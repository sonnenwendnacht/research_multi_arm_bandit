...
def plot_cost_regret(env, alg, algo_choice, T):
    ...
    # Copy the original env.history
    original_history = env.history.copy()

    # Loop over t in range(len(arms_history))
    for t in range(len(arms_history)):
        # Set env.history to the first t+1 events
        env.history = original_history[:t+1]

        # Calculate cumulative regret up to current time
        total_quality_regret, total_cost_regret = env.calculate_regret(reset_history=False)
        cumulative_quality_regret.append(total_quality_regret)
        cumulative_cost_regret.append(total_cost_regret)

    # Restore env.history
    env.history = original_history

    # Plot cumulative cost regret and quality regret versus number of pulls
    plt.figure()
    plt.plot(range(1, len(cumulative_cost_regret) + 1), cumulative_cost_regret, label='Cost Regret')
    plt.plot(range(1, len(cumulative_quality_regret) + 1), cumulative_quality_regret, label='Quality Regret')
    plt.xlabel('Number of Pulls')
    plt.ylabel('Cumulative Regret')
    plt.title(f'Algorithm {algo_choice} - Cumulative Regret vs Number of Pulls')
    plt.legend()
    plt.grid(True)

    # Save the plot as a PNG file
    plot_filename = f'Algorithm_{algo_choice}_Cumulative_Regret.png'
    plt.savefig(plot_filename)
    plt.close()
    print(f"Cumulative regret plot saved as {plot_filename}")
